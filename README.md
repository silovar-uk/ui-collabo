...
**HTMLを貼る**場合は、トップバーの「HTMLを読み込む」からHTMLソースを貼り付けます(または `.html`
ファイルのドロップでも取り込めます)。この場合はスクリーンショットではなく実際のページが表示され、
要素をクリックするだけでCSSセレクタと実測値(文字サイズ・色など)を含む箇所になります。出力はセレクタ
付きの「コード修正指示」になり、渡す先を画像を見るAIからコードを書くAIへ切り替えられます。
...
例外として、HTMLの取り込み画面で「元のCSSも読み込む」を明示的にオンにした場合のみ、
指定した元URLへ通信してスタイルシートや画像を取得します(既定はオフで、通信しません)。
